...
export class AnthropicHandler implements ApiHandler {
  ...
  private ensureClient(): Anthropic {
    if (!this.client) {
      ...
      try {
        ...
        throw new Error(
          `Error creating Anthropic client: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
    return this.client;
  ...
  async *createMessage(
    ...
    const reasoningOn = !!(
      (modelId.includes("3-7") || modelId.includes("4-")) &&
      budget_tokens !== 0
    );
        ...
        stream = await client.messages.create(
          {
            ...
            thinking: reasoningOn
              ...
              : undefined,
            max_tokens: this.options.maxTokens || model.info.maxTokens || 8192,
            // "Thinking isn't compatible with temperature, top_p, or top_k modifications as well as forced tool use."
            // (https://docs.anthropic.com/en/docs/build-with-claude/extended-thinking#important-considerations-when-using-extended-thinking)
            temperature: reasoningOn
              ? undefined
